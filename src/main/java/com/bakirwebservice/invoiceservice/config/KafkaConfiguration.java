...
public class KafkaConfiguration {
    ...
    public ProducerFactory <String, InvoiceRequest> invoiceProducerFactory(){
        ...
        configProps.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers); // Kafka broker adresi
    ...
    public ConsumerFactory<String, InvoiceRequest> invoiceConsumerFactory(){
        Map<String, Object> configProps = new HashMap<>();
        configProps.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers); // Kafka broker adresi
    ...
    //DYNAMIC CREATE INVOICE REQUEST KAFKA TEMPLATE

    @Bean
    public ProducerFactory <String, DynamicInvoiceRequest> createDynamicInvoiceProducerFactory(){
        Map<String, Object> configProps = new HashMap<>();
        configProps.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers); // Kafka broker adresi
        configProps.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, StringSerializer.class);
        configProps.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, JsonSerializer.class);
    ...
    @Bean
    public KafkaTemplate<String, DynamicInvoiceRequest> createDynamicInvoiceKafkaTemplate(){
        KafkaTemplate<String, DynamicInvoiceRequest> kafkaTemplate = new KafkaTemplate<>(createDynamicInvoiceProducerFactory());
        //kafkaTemplate.setProducerListener(new InvoiceKafkaProducerListener());
        return kafkaTemplate;
    }

    @Bean
    public ConsumerFactory<String, DynamicInvoiceRequest> createDynamicInvoiceConsumerFactory(){
        Map<String, Object> configProps = new HashMap<>();
        configProps.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers); // Kafka broker adresi
        configProps.put(ConsumerConfig.GROUP_ID_CONFIG, "dynamic-invoice-processor-group"); // Consumer group ID
        configProps.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        configProps.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, JsonDeserializer.class);
        configProps.put(JsonDeserializer.TRUSTED_PACKAGES, "*"); // Paket güvenliği
        configProps.put(JsonDeserializer.VALUE_DEFAULT_TYPE, "com.bakirwebservice.invoiceservice.api.request.DynamicInvoiceRequest");
        configProps.put(JsonSerializer.ADD_TYPE_INFO_HEADERS, true);
    ...
    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, DynamicInvoiceRequest> dynamicInvoiceRequestConcurrentKafkaListenerContainerFactory(){
        ConcurrentKafkaListenerContainerFactory<String, DynamicInvoiceRequest> factory = new ConcurrentKafkaListenerContainerFactory<>();
        factory.setConsumerFactory(createDynamicInvoiceConsumerFactory());
